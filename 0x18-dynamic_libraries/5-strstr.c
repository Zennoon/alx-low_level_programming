...
/**
 * _strlen_1 - Computes the length of a string
...
/**
 * _strncmp_1 - Compares two strings upto n and returns a number accordingly
 ...
 */
int _strncmp_1(char *s1, char *s2, int n)
{
	int diff, i;
	char *ptr1, *ptr2;
...
char *_strstr(char *haystack, char *needle)
{
	...
	len_s = _strlen_1(haystack) + 1;
	len_ndl = _strlen_1(needle);
	ptr = haystack;
	for (i = 0; i < len_s - len_ndl; i++)
	{
		if (!(_strncmp_1(ptr, needle, len_ndl)))
		{
			return (ptr);
		}
		ptr++;
